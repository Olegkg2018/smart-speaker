...
// Буфер на семьсот миллисекунд. Треть секунды не хватало: плата теперь
// считает эхоподавление и активационное слово, задача динамика получает
// процессор не сразу, и буфер переполнялся — кадры терялись пачками.
// Больше взять нельзя: буфер живёт во внутренней памяти, а её на плате
// около четверти мегабайта, и полутора секунд звука хватило, чтобы плата
// перестала загружаться вовсе.
#define RING_BYTES (BYTES_PER_MS * 700)
...
esp_err_t happy_audio_out_start(void)
{
    ...
    // Приоритет выше, чем у обработки звука с микрофона: пропущенный кадр
    // динамика слышно сразу, а задержка распознавания на слух незаметна.
    if (xTaskCreate(speaker_task, "speaker", 4096, NULL, 9, NULL) != pdPASS) {
        return ESP_FAIL;
    }
...
void happy_audio_out_push(const uint8_t *pcm, size_t len)
{
    ...
    if (sent < len) {
        // Ругаться на каждый кадр бессмысленно: их полсотни в секунду, и
        // лог превращается в сплошную стену, где не видно ничего другого.
        static TickType_t last_warn;
        TickType_t now = xTaskGetTickCount();
        if (now - last_warn > pdMS_TO_TICKS(2000)) {
            last_warn = now;
            ESP_LOGW(TAG, "буфер вывода переполнен, кадры теряются");
        }
    }
}
