...
class Settings(BaseSettings):
    ...
    # --- HTTPS для сателлита ---
    # Браузер открывает микрофон только на защищённой странице, а сателлит
    # на телефоне — это браузер.
    #
    # ВНИМАНИЕ: включение делает HTTPS весь сервер целиком, включая
    # WebSocket. Прошивка колонки ходит по ws:// и после этого
    # подключаться перестанет — ей понадобится wss:// и перепрошивка.
    # Поэтому по умолчанию выключено, а для телефона проще разрешить
    # незащищённый источник флагом в браузере (см. страницу /satellite).
    tls_cert: str = ""
    tls_key: str = ""
